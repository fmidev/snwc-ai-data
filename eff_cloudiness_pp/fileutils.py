...
def read_grib(gribfile, read_coordinates=False, config_path=None):
    ...
    wrk_gribfile = get_local_file1(gribfile, config_path)
...
def get_local_file1(grib_file, config_path):
    if not grib_file.startswith("s3://"):
        return grib_file

    uri = "simplecache::{}".format(grib_file)

    s3_options = {"anon": True, "client_kwargs": {"endpoint_url": "https://lake.fmi.fi"}}

    if config_path:
        # Load the arcus credentials
        access_key = os.getenv("ARCUS_ACCESS_KEY")
        secret_key = os.getenv("ARCUS_SECRET_KEY")
        endpoint_url = os.getenv("ARCUS_ENDPOINT")
    ...
    # Open and return the local path using fsspec
    return fsspec.open_local(uri, mode="rb", s3=s3_options)
